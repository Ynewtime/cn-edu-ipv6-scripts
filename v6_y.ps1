...
$hosts_url = "https://raw.githubusercontent.com/lennylxx/ipv6-hosts/master/hosts" # hosts文件版权遵循相应的项目LICENSE
...
else
{
    ...
    # 1. 更改本机 IPv6 DNS 为 240c::6644, 240c::6666
    ...
    if( ((Get-Date).DayOfWeek -le 5) -and ((Get-Date).Hour -lt 6) ) 
    {
        ...
        # 更改本机所有适配器的 IPv6 DNS 为 240c::6644, 240c::6666
        echo "状态：正在设置 IPv6 DNS"
        Get-DnsClientServerAddress -AddressFamily IPv6 |
        foreach {
            Set-DnsClientServerAddress -InterfaceIndex $_.InterfaceIndex -Addresses '240c::6644','240c::6666'
        }
